...
# 添加学号尾数映射和生成逻辑
student_id = "202210179005"
last_digit = int(student_id[-1])  # 提取末位数字
...
# 获取目标国家索引
target_nationality = nationality_mapping[last_digit]
try:
    nationality_index = vectorizer.nationality_vocab.lookup_token(target_nationality)
except KeyError:
    raise ValueError(f"数据集未包含'{target_nationality}'国家，请检查训练数据")
...
sampled_indices = sample_from_model(
    model,
    vectorizer,
    nationalities=[nationality_index] * 5,  # 重复5次相同国家索引
    sample_size=20,
...
for i, name in enumerate(decoded_surnames):
    print(f"{i+1}. {name}")
